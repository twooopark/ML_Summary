...
hold lot(멈춤) 
  ...
  검증은 어떻게 할 것인가? : 교차검증(CV), K-ford, Leave One Out
  - CV : 진성 30개, 가성 920개 가 있다?(총 950개의 데이터) >> 가성 920를 30개의 subset과 20개의 버리는 set으로 나누고, 진성 30개와 가성 subset들로 총 30회 학습을 한다.? No,,, training 독립변수 10개, test 종속변수 1개라 총 30*10 300번의 검증이 필요.
  - 결과로 정확도 A:76%, B:72%, C:86% 가 나왔다. 
  - 학습 데이터가 3달분이라 믿을 수 있는지 모르겠다. >  트레이닝 데이터에 없는 테스트 데이터를 넣어보니 결과가 좋지 않았다.
  - 그래서,,, ....
  
  - Confusion Matrix로 정확도를 구할 수 있다. 정확도=(TP+TN)/(TP+TN+ FP+FN)
  - 위 예시(진성,가성) 에서 진성 : TN, FP 가성 : TP, FN
...
이러한 머신러닝의 최종적인 목표는 인간의 뇌와 유사하게 만드는 것입니다.
  ...
  - 데이터를 기반으로, 데이터를 보고 명시적으로 정의되지 않은 패턴을 컴퓨터로 학습하여 예측하는 것이 머신러닝의 핵심이다.
```
